...
template <typename vtype> struct vec3 {
  vec3() { X[0] = X[1] = X[2] = 0; }
  ...
  vec3 operator*(vtype d) const { return vec3(X[0] * d, X[1] * d, X[2] * d); }
  ...
  void print() { cout << X[0] << " " << X[1] << " " << X[2] << endl; }

  bool zero() { return abs(X[0]) < eps && abs(X[1]) < eps && abs(X[2]) < eps; }
...
struct face {
  ...
  face() {}
  face(int i, int j, int k) { idx[0] = i, idx[1] = j, idx[2] = k; }
...
void mass_center(vector<point> &cloud, vector<face> &faces) {
  ...
  for (auto f : faces) {
    auto value = (cloud[f[0]] - pivot)
                     .dot((cloud[f[1]] - pivot) * (cloud[f[2]] - pivot));
