...
bool rpn(const vector<token> &tokens, queue<token> &rpn) {
  ...
  for (auto &token : tokens) {
    if (token.ttype == value)
      ...
      while (operators.size() > 0 && operators.top().ttype != obracket &&
             operations[token.text].precedence >
                 operations[operators.top().text].precedence) {
...
template <typename T> T eval(queue<token> &rpn, bool &ok) {
  stack<T> result;
  while (rpn.size() > 0) {
    auto t = rpn.front();
    rpn.pop();
